...
# Class factories
* generating classes on demand that extend a variable superclass
```javascript
function makeT(S = Object) {
  return class T extends S {
    transmit() {
...
function makeR(S = Object) {
  return class R extends S {
    receive() {
        console.log("receive");
    }
  };
