...
if (!Promise.map) {
	Promise.map = (vals, cb) => // new promise that waits for all mapped promises
    Promise.all(// note: regular array `map(..)`, turns
    // the array of values into an array of
    // promises
    vals.map( val => // replace `val` with a new promise that
